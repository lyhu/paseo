...
import { createClientChannel, type EncryptedChannel } from "@getpaseo/relay/e2ee";
import {
  decodeTunnelFrame,
  encodeTunnelFrame,
  TunnelCreditWindow,
...
export class EgressRuntime {
  ...
  constructor(options: EgressRuntimeOptions) {
    ...
    this.#server.on("connect", (_request, socket) => rejectUnsupportedProtocol(socket));
    this.#server.on("upgrade", (_request, socket) => rejectUnsupportedProtocol(socket));
  }
  ...
  async #handleE2EEOpen(connectionId: string, req: IncomingMessage): Promise<void> {
    const conn = this.#dataConnections.get(connectionId);
    if (!conn?.channel) return;

    const blockedHeaders = new Set(["host"]);
    if (this.#access.mode === "header") blockedHeaders.add("x-paseo-access-token");
    const headers = sanitizeTunnelHeaders(rawHeadersToTuples(req.rawHeaders), blockedHeaders);
  ...
  #handleMessage(connectionId: string, data: string | ArrayBuffer, res: ServerResponse): void {
    const conn = this.#dataConnections.get(connectionId);
    if (!conn) return;
  ...
  #handleControlFrame(connectionId: string, frame: TunnelFrame, res: ServerResponse): void {
    const conn = this.#dataConnections.get(connectionId);
    if (!conn) return;
