...
export interface TunnelSubsystemOptions {
	configStore: Pick<
		DaemonConfigStore,
		"getPersistedConfigSnapshot" | "setPersistedTunnelConfig"
	>;
...
export class TunnelSubsystem {
	...
	async #updateIngress(options: UpdateIngressOptions): Promise<MutationResult> {
		const config = this.#loadConfig();
		const ingress = config.daemon?.tunnel?.ingresses?.find(
			(i) => i.id === options.id,
		);
	...
	async #deleteIngress(id: string): Promise<MutationResult> {
		const config = this.#loadConfig();
		if (!config.daemon?.tunnel?.ingresses)
			throw new Error(`Ingress ${id} not found`);
	...
	async #createEgress(options: CreateEgressOptions): Promise<MutationResult> {
		...
		if (
			current.daemon?.tunnel?.egresses?.some(
				(egress) => egress.name === options.name,
			)
		) {
	...
	async #updateEgress(options: UpdateEgressOptions): Promise<MutationResult> {
		...
		const egress = config.daemon?.tunnel?.egresses?.find(
			(e) => e.id === options.id,
		);
	...
	async #deleteEgress(id: string): Promise<MutationResult> {
		const config = this.#loadConfig();
		if (!config.daemon?.tunnel?.egresses)
			throw new Error(`Egress ${id} not found`);
	...
	async #syncIngressRuntime(options: {
		...
		const config = this.#loadConfig();
		const nextRuntime = await this.#startIngressRuntime(
			config.daemon?.tunnel,
			options,
		);
	...
	#generateAccessToken(): string {
		return `ptt-${randomBytes(8).toString("hex")}`;
	}
