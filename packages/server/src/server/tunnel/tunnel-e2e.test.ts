...
describe("Tunnel E2E", () => {
  ...
  beforeAll(async () => {
    ...
    if (!addr || typeof addr === "string") throw new Error("Invalid target address");
    targetPort = addr.port;

    subsystem = new TunnelSubsystem({
      configStore: createConfigStore(paseoHome),
  ...
  it("forwards JSON request/response through E2EE relay", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${egress.listen.port}/echo`, {
      method: "POST",
  ...
  it("preserves repeated headers and rebuilds proxy headers", async () => {
    ...
    const observed = await new Promise<{
      ...
      const req = httpRequest(
        ...
        async (response) => {
          ...
          resolve({ body: Buffer.concat(chunks).toString(), rawHeaders: response.rawHeaders });
        },
      );
    ...
    expect(observed.rawHeaders).toEqual(
      expect.arrayContaining(["set-cookie", "first=1", "set-cookie", "second=2"]),
    );
    expect(observed.rawHeaders.map((value) => value.toLowerCase())).not.toContain(
      "x-response-drop",
    );
  });
  ...
  it("validates access token in header mode", async () => {
    ...
    const egress = egressResult.state.egresses.find((e) => e.name === "auth-egress")!;

    // Request without token should fail
    const noTokenResponse = await fetch(`http://127.0.0.1:${egress.listen.port}/echo`, {
      method: "POST",
      body: "test",
    });
    ...
    const wrongTokenResponse = await fetch(`http://127.0.0.1:${egress.listen.port}/echo`, {
      method: "POST",
    ...
    const goodResponse = await fetch(`http://127.0.0.1:${egress.listen.port}/echo`, {
      method: "POST",
    ...
    const result = (await goodResponse.json()) as { bytes: number; sha256: string };
    expect(result.bytes).toBe(body.byteLength);
    expect(result.sha256).toBe(expectedHash);
  }, 10_000);
  ...
  it("returns a fixed 502 for an invalid route capability", async () => {
    ...
    const egress = result.state.egresses.find((item) => item.name === "invalid-route-egress")!;
  ...
  it("returns a fixed 502 when the target is unavailable", async () => {
    ...
    if (!address || typeof address === "string") throw new Error("Invalid unavailable address");
