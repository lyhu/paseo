...
export function decodeTunnelFrame(raw: string): TunnelFrame {
  ...
  switch (value.type) {
    ...
    case "request.ack":
    case "response.ack":
      if (isChunkByteLength(value.bytes)) return { v: 1, type: value.type, bytes: value.bytes };
      break;
    case "request.end":
    case "response.end":
      return { v: 1, type: value.type };
    case "error":
      if (isTunnelErrorCode(value.code)) return { v: 1, type: value.type, code: value.code };
      break;
  }
  throw new Error("Invalid Tunnel frame");
}
...
function decodeRequestHead(value: Record<string, unknown>): TunnelFrame {
  ...
  if (typeof value.routeId !== "string") throw new Error("Invalid Tunnel frame");
  if (typeof value.routeSecret !== "string") throw new Error("Invalid Tunnel frame");
...
function decodeResponseHead(value: Record<string, unknown>): TunnelFrame {
  if (typeof value.statusCode !== "number") throw new Error("Invalid Tunnel frame");
  if (!Number.isInteger(value.statusCode)) throw new Error("Invalid Tunnel frame");
  ...
  if (typeof value.statusMessage === "string") frame.statusMessage = value.statusMessage;
  return frame;
}
...
function isTunnelErrorCode(value: unknown): value is TunnelErrorCode {
  return typeof value === "string" && TUNNEL_ERROR_CODES.some((code) => code === value);
}
